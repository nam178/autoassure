...
) : ControllerBase
{
    ...
    )
    {
        if (
            ...
            return BadRequest(
                new ErrorResponse(
                    "ActivityResult.Status must be Passed, Failed or Skipped."
                )
            );
        ...
        return appended
            ...
            : Conflict(
                new ErrorResponse(
                    "Seq has already been used, or the Run's Status is not Running."
                )
            );
    }
    ...
    public async Task<
        ActionResult<IReadOnlyList<RunStatusUpdateResponse>>
    > List(Guid applicationId, Guid runId, [FromQuery] long after = 0)
    {
        if (after < 0)
