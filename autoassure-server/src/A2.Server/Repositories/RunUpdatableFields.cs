...
/// field is left unchanged; set a field to write it. At least one field must be set.</summary>
public record RunUpdatableFields
{
    ...
    /// <see cref="SkippedActivityCount"/>; nothing that needs exact progress reads these counts, since
    /// that caller folds the status update log instead.</summary>
