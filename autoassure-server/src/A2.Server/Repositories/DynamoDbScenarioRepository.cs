...
public class DynamoDbScenarioRepository(
...
) : IScenarioRepository
{
    ...
    private const string FolderPartitionKeyName =
        "OrganizationId_ApplicationId_Folder";

    private const string TagPartitionKeyName =
        "OrganizationId_ApplicationId_Tag";

    private string ScenarioTableName => options.Value.ScenarioTableName;

    private string ScenariosByFolderTableName =>
        options.Value.ScenariosByFolderTableName;

    private string ScenariosByTagTableName =>
        options.Value.ScenariosByTagTableName;
    ...
    )
    {
        ...
        {
            transactItems.Add(
                DeleteFolderMapping(previousState, previousState.Folder)
            );
            transactItems.Add(PutFolderMapping(scenario, scenario.Folder));
        }
        ...
        transactItems.AddRange(
            previousTags
                .Except(currentTags)
                .Select(tag => DeleteTagMapping(previousState, tag))
        );
        transactItems.AddRange(
            currentTags
                .Except(previousTags)
                .Select(tag => PutTagMapping(scenario, tag))
        );
    ...
    )
    {
        ...
        foreach (var chunk in scenarioIds.Chunk(BatchGetChunkSize))
        {
            var requestItems = new Dictionary<string, KeysAndAttributes>
            {
                ...
                {
                    Keys = chunk
                        .Select(scenarioId => new Dictionary<
                            string,
                            AttributeValue
                        >
                        {
            ...
            while (requestItems.Count > 0)
            {
                ...
                if (
                    response.Responses.TryGetValue(
                        ScenarioTableName,
                        out var items
                    )
                )
    ...
    )
    {
                ...
                RequestItems = new Dictionary<string, KeysAndAttributes>
                {
                    [ScenarioTableName] = new()
                    {
                        Keys = mappingResponse
                            .Items.Select(item => new Dictionary<
                                string,
                                AttributeValue
                            >
                            {
        ...
        return scenarioIds
            .Select(scenarioId => scenariosById[scenarioId].ToScenario())
            .ToList();
    }
        ...
        return new TransactWriteItem
        {
            Update = new Update
            {
                ...
                >
                {
                    ...
                    [":updatedByUserId"] = new(
                        scenario.UpdatedByUserId.ToString()
                    ),
    ...
    private TransactWriteItem PutFolderMapping(Scenario scenario, string folder)
    {
        return new TransactWriteItem
        {
            Put = new Put
            {
                ...
                {
                    [FolderPartitionKeyName] = new(
                        FolderPartitionKey(scenario, folder)
                    ),
                    ["ScenarioId"] = new(scenario.Id.ToString()),
                    ["OrganizationId"] = new(
                        scenario.OrganizationId.ToString()
                    ),
    ...
    {
        return $"{scenario.OrganizationId}_{scenario.ApplicationId}_{tag}";
    }
}
