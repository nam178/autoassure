...
/// <summary>
///     Assembles the snapshot rows a Run is created with, by reading the live
///     Scenarios, their
///     Activities, the Preconditions/EvidenceDefinitions those Activities
///     reference, and an Environment's
///     variables, then copying each into its dedicated snapshot type (see the
///     snapshot models in
///     <c>Models/</c>).
/// </summary>
...
{
    /// <summary>
    ///     Copies <paramref name="environment" /> and its current variables into a
    ///     <see cref="RunEnvironmentSnapshot" />, value included whole even for a
    ///     sensitive variable -- a
    ///     future execution agent needs the real credential from this snapshot to
    ///     connect to systems under
    ///     test. Nothing is masked here: masking happens later, when the Run is
    ///     claimed (see
    ///     <c>IRunRepository.TryMarkAsStartedAsync</c>) and at the response-mapping
    ///     boundary (see
    ///     <c>ContractMapper.ToResponse(Run, bool)</c>).
    /// </summary>
